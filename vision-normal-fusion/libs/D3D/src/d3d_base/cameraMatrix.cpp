...
void CameraMatrix<NumericType>::loadFromDepthMapDataFile(std::string fileName)
{
    ...
    {
        D3D_THROW_EXCEPTION("Current platform does not have the same endian as the depth map data file.")
    }
    ...
    {
        D3D_THROW_EXCEPTION("Size of numeric data type does not match.")
    }
    ...
    if (!inStream.good())
    {
        D3D_THROW_EXCEPTION("Error while reading the camera matrix")
    }
