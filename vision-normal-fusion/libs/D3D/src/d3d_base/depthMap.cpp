...
namespace D3D
{
    ...
    template <typename T, typename U>
    T* D3D::DepthMap<T, U>::getDataPtr() const
    {
        return depths.get();
    }

    template <typename T, typename U>
    D3D::DepthMap<T,U> D3D::DepthMap<T,U>::clone() const
    {
        D3D::DepthMap<T,U> result(getWidth(), getHeight(), getCam());
        memcpy((void*)result.getDataPtr(), (void*)getDataPtr(), sizeof(T)*getWidth()*getHeight());
        return result;
    }
