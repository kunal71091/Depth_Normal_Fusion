...
{
    // Depth
    int numBytes = xRes*yRes*zRes*sizeof(float);
    *voxelsD = new float[xRes*yRes*zRes];
    memset(*voxelsD, 0, numBytes);
    // Label
    numBytes = numLabels*xRes*yRes*zRes*sizeof(float);
    *voxelsLD = new float[numLabels*xRes*yRes*zRes];
    memset(*voxelsLD, 0, numBytes);
}
...
void downloadTileDebug(int numLabels, int xRes, int yRes, int zRes, float* voxelsD, float* voxelsH, float* voxelsLD, float* voxelsLH)
{
    ...
    // Label
    numBytes = numLabels*xRes*yRes*zRes*sizeof(float);
    memcpy(voxelsLH, voxelsLD, numBytes);
}
            ...
            for (int z = 0; z < zRes; ++z) {
                ...
                if (voxCenterProj[2] >= minDepth && voxCenterProj[2] <= maxDepth)
                {
                    ...
                    if (xp >= 0 && xp < cols && yp >= 0 && yp < rows) {
                        ...
                        int minLabel = labelsMinD[yp*cols + xp];
...
//                            continue;
                        ...
                        if (depth > 0)
                        {
                            ...
                            {
                                const int voxInd = z*xRes*yRes + y*xRes + x;
                                const int voxIndL = (z*xRes*yRes + y*xRes + x)*numLabels; // + label;

                                // Eyeballs
                                if (minLabel == 4) {
                                    if (fabs(dist) < delta3) {
//                                        for (int l = 0; l < numLabels; ++l) {
//                                            voxelLD[voxIndL+l] += labelsD[l*cols*rows + yp*cols + xp];
//                                        }

                                        voxelLD[voxIndL+minLabel]++;
                                    }
                                    // Do not integrate depth
                                    continue;
                                }
...
void intergrateFaceDepthLabels(float* voxelD, float* voxelLD, float* boxToGlobalD,
                               int numLabels, int xRes, int yRes, int zRes,
                               float xBegin, float yBegin, float zBegin,
                               float deltaX, float deltaY, float deltaZ,
                               float* depthDataD, float* labelsD, float* minLabelsD, int rows, int cols,
                               float maxDepth, float minDepth,
                               float epsilon, float eta, float delta1, float delta2, float delta3,
                               float* projD)
{
...
//    D3D_CUDA_CHECK_ERROR
    ...
    integrateFaceDepthLabelsDebug(voxelD, voxelLD, boxToGlobalD,
                                  numLabels, xRes, yRes, zRes,
                                  xBegin, yBegin, zBegin,
                                  deltaX, deltaY, deltaZ,
                                  depthDataD, labelsD, minLabelsD, rows, cols,
                                  maxDepth, minDepth,
                                  epsilon, eta, delta1, delta2, delta3,
                                  projD);
