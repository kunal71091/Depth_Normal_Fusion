#include "cudaDepthIntegratorMultiClassv2.h"
#include <d3d_base/exception.h>
#include <d3d_base/grid.h>
#include <iostream>


using namespace D3D_CUDA::CudaDepthIntegratorMultiClassv2DeviceCode;
using namespace D3D;

CudaDepthIntegratorMultiClassv2::CudaDepthIntegratorMultiClassv2(int xRes, int yRes, int zRes,
...
CudaDepthIntegratorMultiClassv2::~CudaDepthIntegratorMultiClassv2()
{
    if (currentTile >= 0)
...
void CudaDepthIntegratorMultiClassv2::setTile(int i)
{
    if (currentTile >= 0)
        releaseDeviceMemory(voxelsTD);
...
void CudaDepthIntegratorMultiClassv2::addDepthMap(D3D::DepthMap<float, double> &depthMap, D3D::Grid<float>& classScores)
{
    // check dimensions
...
float CudaDepthIntegratorMultiClassv2::getTileX()
{
    return tileX*_numClasses;
}

float CudaDepthIntegratorMultiClassv2::getTileY()
{
    return tileY;
}

float CudaDepthIntegratorMultiClassv2::getTileZ()
{
    return tileZ;
}

D3D::Grid<float> CudaDepthIntegratorMultiClassv2::downloadTile()
{
    D3D::Grid<float> voxels(xResT*_numClasses, yResT, zResT);
    D3D_CUDA::CudaDepthIntegratorMultiClassv2DeviceCode::downloadTile(xResT*_numClasses, yResT, zResT, voxelsTD, voxels.getDataPtr());
