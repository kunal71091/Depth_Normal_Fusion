...
using namespace D3D_CUDA::CudaFishEyePlaneSweepDeviceCode;
...
int CudaFishEyePlaneSweep::addDeviceImage(D3D_CUDA::DeviceImage& image, FishEyeCameraMatrix<double>& cam)
{
    CudaFishEyePlaneSweepImage cPSI;
    cPSI.devImg = image;
    cPSI.cam = cam;
...
//    this->planes = planes; // TODO: check if this did not break anything
...
#ifdef USE_FACT_LUT
    {
        ...
        factLUT.reallocatePitched(refImg.devImg.getWidth(), refImg.devImg.getHeight());
        planeSweepComputeFactLUT(KrefInvA, refImg.cam.getXi(), factLUT);
    }
#endif
        ...
        for (std::map<int, CudaFishEyePlaneSweepImage>::iterator it = images.begin(); it != images.end(); it++)
        {
            ...
            case FISH_EYE_PLANE_SWEEP_OCCLUSION_BEST_K:
            {
                ...
                if (boxFilterBeforeOcclusion && matchingCosts != FISH_EYE_PLANE_SWEEP_ZNCC)
                {
                    ...
                    D3D_CUDA::DeviceBuffer<float> temp = boxFilterTempBuffer;
