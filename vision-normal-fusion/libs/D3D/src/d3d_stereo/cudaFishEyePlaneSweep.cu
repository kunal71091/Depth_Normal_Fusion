...
#include <d3d_cudaBase/deviceBuffer.cuh>

#include <d3d_cudaBase/deviceImage.h>
#include <d3d_cudaBase/deviceImage.cuh>

#include <vector>

//#define ENABLE_TANGO_TEST_HACK
...
namespace D3D_CUDA
{
    namespace CudaFishEyePlaneSweepDeviceCode
    {
        ...
        {
            // Bind texture
            D3D_CUDA_CHECKED_CALL( cudaBindTexture2D(0, planeSweepCostTexture, costBuf.getAddr(), planeSweepCostChannelDesc, costBuf.getWidth(), costBuf.getHeight(), costBuf.getPitch()); )
            ...
            // unbind texture
            D3D_CUDA_CHECKED_CALL( cudaUnbindTexture(planeSweepCostTexture); )
        }
                                                 ...
                                                 DeviceBuffer<float>& tempBuf, DeviceBuffer<float>& tempSqrBuf, int radius_x, int radius_y)
        {
            ...
            // bind texture
            D3D_CUDA_CHECKED_CALL( cudaBindTexture2D(0, planeSweepGrayscaleTextureNonInterp, refImg.getAddr(), planeSweepGrayscaleChannelDesc, refImg.getWidth(), refImg.getHeight(), refImg.getPitch()); )
            ...
            D3D_CUDA_CHECKED_CALL(cudaUnbindTexture(planeSweepGrayscaleTextureNonInterp))
        }
        ...
        {
            // bind textures
            D3D_CUDA_CHECKED_CALL( cudaBindTexture2D(0, planeSweepGrayscaleTexture, otherImg.getAddr(), planeSweepGrayscaleChannelDesc, otherImg.getWidth(), otherImg.getHeight(), otherImg.getPitch()); )
            D3D_CUDA_CHECKED_CALL( cudaBindTexture2D(0, planeSweepGrayscaleTextureNonInterp, refImg.getAddr(), planeSweepGrayscaleChannelDesc, refImg.getWidth(), refImg.getHeight(), refImg.getPitch()); )
            ...
            // unbind textures
            D3D_CUDA_CHECKED_CALL( cudaUnbindTexture(planeSweepGrayscaleTexture) )
            D3D_CUDA_CHECKED_CALL( cudaUnbindTexture(planeSweepGrayscaleTextureNonInterp) )
        }
                                                        ...
                                                        DeviceImage refImg, DeviceBuffer<float> costBuf)
        {
            ...
            if (x < width && y < height)
            {
#ifdef ENABLE_TANGO_TEST_HACK
                // apply KrefInv

                float mx = krefinv11*x + krefinv13;
                float my = krefinv22*y + krefinv23;

                float rd = sqrt(mx*mx + my*my);
                float ru = tan(rd*xiref)/(2.0f*tan(xiref/2.0f));
                ...
                float ruw = sqrt(xxw*xxw + yyw*yyw);
                float rdw = 1.0f/xiother*atan(2.0f*ruw * tan(xiother/2.0f));
                ...
                xw = kother11*xw + kother13;
                yw = kother22*yw + kother23;
...
//                float id = fabs((float)(i));
                              ...
                              const DeviceImage &refImg, DeviceBuffer<float>& costBuf)
        {
            ...
            D3D_CUDA_CHECKED_CALL( cudaBindTexture2D(0, planeSweepGrayscaleTexture, srcImg.getAddr(), planeSweepGrayscaleChannelDesc, srcImg.getWidth(), srcImg.getHeight(), srcImg.getPitch()); )
...
//                float id = fabs((float)(i));
                                         ...
                                         float* bestDepthsAddr, int bestDepthsPitch, std::vector<float> &KrefInv, float XiRef)
        {
            ...
            float* planesAddr;
            size_t planesPitch;
            D3D_CUDA_CHECKED_CALL( cudaMallocPitch(&planesAddr, &planesPitch, sizeof(float)*numPlanes, 4); )
            // copy planes to device
            D3D_CUDA_CHECKED_CALL( cudaMemcpy2D(planesAddr, planesPitch, &(planes[0]), sizeof(float)*numPlanes, sizeof(float)*numPlanes, 4, cudaMemcpyHostToDevice); )

            // allocate memory on device for best depths
            float* bestDepthsDAddr;
            size_t bestDepthsDPitch;
            D3D_CUDA_CHECKED_CALL( cudaMallocPitch(&bestDepthsDAddr, &bestDepthsDPitch, sizeof(float)*width, height); )
            ...
            D3D_CUDA_CHECKED_CALL( cudaMemcpy2D(bestDepthsAddr, bestDepthsPitch, bestDepthsDAddr, bestDepthsDPitch, sizeof(float)*width, height, cudaMemcpyDeviceToHost); )

            // free device memory
            D3D_CUDA_CHECKED_CALL( cudaFree(planesAddr); )
            D3D_CUDA_CHECKED_CALL( cudaFree(bestDepthsDAddr); )
