...
add_vertex(PointIdx _p0, PointIdx _p1)
{
  ...
  { 
    ...
    std::cout << "Scalar Values:" << s0 << ", " << s1 << std::endl;
    if(sign(s0) != sign(s1)) // if sign does not change not interesting
    {
      float t  = fabs(s0) / (fabs(s0)+fabs(s1));
      float t1 = 1.0-t; 
      point = t1*p0 + t*p1;

      float dx = grid_.dx().norm();
      float dy = grid_.dx().norm();
      float dz = grid_.dx().norm();

      std::cout << point[0] + dx << std::endl;
      std::cout << point[1] + dy << std::endl;
      std::cout << point[2] + dz << std::endl;

      float grad_x = grid_.lerp_local(point[0] + dx, point[1], point[2]) - grid_.lerp_local(point[0] - dx, point[1], point[2]);
      float grad_y = grid_.lerp_local(point[0], point[1] + dy, point[2]) - grid_.lerp_local(point[0], point[1] - dy, point[2]);
      float grad_z = grid_.lerp_local(point[0], point[1], point[2] + dz) - grid_.lerp_local(point[0], point[1], point[2] - dz);

      typename Mesh::Point normal(grad_x, grad_y, grad_z);
      normal.normalize();
    ...
    {
      std::cout << "My sign did not change, no intersection!" << std::endl;
    }
  }
